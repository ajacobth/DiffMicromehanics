...
OUTPUT_UNITS: Dict[str, Optional[str]] = {
    ...
    "nu12": "-", "nu13": "-", "nu23": "-",
    "CTE11": r"$\mu$/K", "CTE22": r"$\mu$/K", "CTE33": r"$\mu$/K",
    "CTE12": r"$\mu$/K", "CTE13": r"$\mu$/K", "CTE23": r"$\mu$/K",
}
...
OUTPUT_DISPLAY_NAMES: Dict[str, str] = {
    ...
    "nu12": r"$\nu_{12}$", "nu13": r"$\nu_{13}$", "nu23": r"$\nu_{23}$",
    "CTE11": r"$\mathrm{CTE}_{11}$", "CTE22": r"$\mathrm{CTE}_{22}$", "CTE33": r"$\mathrm{CTE}_{33}$",
    "CTE12": r"$\mathrm{CTE}_{12}$", "CTE13": r"$\mathrm{CTE}_{13}$", "CTE23": r"$\mathrm{CTE}_{23}$",
}
...
def evaluate(config: ml_collections.ConfigDict, workdir: str):
    ...
    test_data = np.genfromtxt("THELPSD_dataset_test_V2.csv", delimiter=",", skip_header=1)
